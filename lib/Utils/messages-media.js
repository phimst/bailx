...
async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        ...
        const imgFilename = (0, path_1.join)(getTmpFilesDirectory(), (0, generics_js_1.generateMessageIDV2)() + '.jpg');
        try {
...
const uploadWithNodeHttp = async ({ url, filePath, headers, timeoutMs, agent }, redirectCount = 0) => {
    ...
    const httpModule = parsedUrl.protocol === 'https:' ? await require('https') : await require('http');
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
        ...
        for (const { hostname } of hosts) {
            ...
            const url = `https://${hostname}${index_js_2.MEDIA_PATH_MAP[mediaType]}/${fileEncSha256B64}?auth=${auth}&token=${fileEncSha256B64}`;
            let result;
            try {
