...
const presence_manager_js_1 = require("../Utils/presence-manager.js");
...
const makeWASocket = (config) => {
    ...
    // --- seraphbail: reactive presence manager --------------------------------
    // Idle by default, flips online only when a real inbound message arrives,
    // goes back idle after a few minutes of silence. Auto-wired, opt-out via
    // enablePresenceManager: false.
    if (newConfig.enablePresenceManager !== false) {
        const presenceManager = (0, presence_manager_js_1.createPresenceManager)(sock, newConfig.presenceManagerOptions);
        sock.presenceManager = presenceManager;
        ...
        sock.ev.on("messages.upsert", ({ messages, type }) => {
            ...
            const hasInbound = messages.some(m => m.message && !m.key.fromMe);
            if (hasInbound) presenceManager.notifyActivity();
        });
    }
